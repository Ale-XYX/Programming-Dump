...
class Application:
    def __init__(self):
        ...
        self.encode_btn = tk.Button(self.encode_frame, text='Encode!', command=lambda: Functions.encode(self.encode_entry, Public.random_active, self.encode_rotationbox, self.encode_output, self.encode_btn))
        self.encode_copy_btn = tk.Button(self.encode_frame, text='Copy', command=lambda: Functions.encode_copy(self.encode_output))
        self.encode_clear_btn = tk.Button(self.encode_frame, text='Clear', command=lambda: Functions.encode_clear(self.encode_output, self.encode_btn))
        ...
        self.decode_rotationbox = ttk.Spinbox(self.decode_frame, increment=1, from_=0, to_=93)
        self.decode_text = ttk.Label(self.decode_frame, text='Place text to decode here', font=self.standard_font)
        self.decode_entry = ttk.Entry(self.decode_frame)
        self.decode_output = tkText.ScrolledText(self.decode_frame)
        self.decode_output_text = ttk.Label(self.decode_frame, text='Output at rotation', font=self.standard_font)
        self.decode_output_rotationbox = ttk.Spinbox(self.decode_frame, increment=1, from_=0, to_=93, textvariable=self.output_rotationbox_status, command=lambda: Functions.decode_update(self.decode_output, self.output_rotationbox_status), wrap=True)
        self.decode_btn = tk.Button(self.decode_frame, text='Decode!', command=lambda: Functions.decode(self.decode_entry, Public.brute_active, self.decode_rotationbox, self.decode_output_rotationbox, self.decode_output, self.output_rotationbox_status, self.decode_btn))
        self.decode_brute_checkbutton = ttk.Checkbutton(self.decode_frame, text='Brute Force', variable=self.bbutton_status, command=lambda: Functions.bbutton_check(self.bbutton_status))
        self.decode_copy_btn = ttk.Button(self.decode_frame, text='Copy', command=lambda: Functions.decode_copy(self.decode_output))
        self.decode_clear_btn = ttk.Button(self.decode_frame, text='Clear', command=lambda: Functions.decode_clear(self.decode_output, self.decode_output_rotationbox, self.bbutton_status, self.decode_btn))

        self.main_encode_activate = tk.Button(self.root, text='Encode', command=lambda: Functions.switch_scenes(True, False, self.main_encode_activate, self.main_decode_activate, self.encode_frame, self.decode_frame))
        self.main_decode_activate = tk.Button(self.root, text='Decode', command=lambda: Functions.switch_scenes(False, True, self.main_encode_activate, self.main_decode_activate, self.encode_frame, self.decode_frame))
